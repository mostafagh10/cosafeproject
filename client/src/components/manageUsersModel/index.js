...
const ShowManageUserModel = () => {
  ...
  */
  ...
  const showTheItems = () => (
    <div>
    <div>
    <input className="form-control filteradmin" style={{float:'left'}} placeholder="Filter The Users (by email)..." onChange={handletextsearch} />
    </div><br /><br />
    <table>
        <thead>
            <th>first name</th>
        ...
        {users && users.map(user => (
              ...
              <td data-label="action">
                <a onClick={() => {window.location.href=`/admin/manage/edituser/${user._id}`}}>
                <button className="btn btn-success text-white" ><i className="fas fa-edit"></i> make actions</button> 
                </a> 
              </td>
              </tr>
           ))}
