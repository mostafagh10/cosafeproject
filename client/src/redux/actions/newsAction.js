...
export const GET_NEWS = (newsId) => async dispatch => {
    try {
        const response = await axios.get(`https://cosafebackend3.onrender.com/news/${newsId}`)
...
export const DELETE_NEWS = newsId => async dispatch => {
    try {
        const response = await axios.delete(`https://cosafebackend3.onrender.com/news/${newsId}`)
