...
export const GET_USER = (userId) => async dispatch => {
    try {
        const response = await axios.get(`https://cosafebackend3.onrender.com/user/client/${userId}`)
...
export const DELETE_USER = userId => async dispatch => {
    try {
        const response = await axios.delete(`https://cosafebackend3.onrender.com/user/client/${userId}`)
