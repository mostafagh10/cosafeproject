...
const EditAdmin = ({ match }) => {
  ...
  const makedeleteprocess = async() => {
    await axios.delete(`https://cosafebackend3.onrender.com/user/admin/${adminId}`).then(() => {
      history.push('/admin/manage/manageadmins')
    }).catch((err) => {
      history.push('/admin/manage/manageadmins')
    })
    ...
    const Showtheform = () => (
        ...
        <div className="modal-content">
          ...
          <div className="modal-footer">
            <button type="submit" className="btn btn-danger text-white">Update</button>
          </div>
          </form>
        </div>
        </div>
    ...
    const deleteform = () => {
      return(
        <div className='container my-3'>
          <div className="row">
        <div className="col-md-8 mx-auto">
        <button className='btn btn-danger' onClick={makedeleteprocess}><i className='fas fa-trash'></i> Delete The Admin</button>  
        </div>
        </div>    
        </div>
      )
