...
export const GET_ADVICE = (adviceId) => async dispatch => {
    try {
        const response = await axios.get(`https://cosafebackend3.onrender.com/advice/${adviceId}`)
...
export const DELETE_ADVICE = adviceId => async dispatch => {
    try {
        const response = await axios.delete(`https://cosafebackend3.onrender.com/advice/${adviceId}`)
