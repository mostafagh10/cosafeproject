...
const EditAdvice = ({ match }) => {
  ...
  const makedeleteprocess = async() => {
    await axios.delete(`https://cosafebackend3.onrender.com/advice/${adviceId}`).then(() => {
      history.push('/admin/manage/manageadvices')
    }).catch((err) => {
      history.push('/admin/manage/manageadvices')
    })
    ...
    const deleteform = () => {
      return(
        <div className='container my-3'>
          <div className="row">
        <div className="col-md-8 mx-auto">
        <button className='btn btn-warning' onClick={makedeleteprocess}><i className='fas fa-trash'></i> Delete The Advice</button>  
        </div>
        </div>    
        </div>
      )
