...
const EditUser = ({ match }) => {
  ...
  const makedeleteprocess = async() => {
    await axios.delete(`https://cosafebackend3.onrender.com/user/client/${userId}`).then(() => {
      history.push('/admin/manage/manageusers')
    }).catch((err) => {
      history.push('/admin/manage/manageusers')
    })
    ...
    const deleteform = () => {
      return(
        <div className='container my-3'>
          <div className="row">
        <div className="col-md-8 mx-auto">
        <button className='btn btn-success' onClick={makedeleteprocess}><i className='fas fa-trash'></i> Delete The User</button>  
        </div>
        </div>    
        </div>
      )
