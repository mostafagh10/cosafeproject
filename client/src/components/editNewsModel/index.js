...
const EditNews = ({ match }) => {
  ...
  const makedeleteprocess = async() => {
    await axios.delete(`https://cosafebackend3.onrender.com/news/${newsId}`).then(() => {
      history.push('/admin/manage/managenews')
    }).catch((err) => {
      history.push('/admin/manage/managenews')
    })
    ...
    const deleteform = () => {
      return(
        <div className='container my-3'>
          <div className="row">
        <div className="col-md-8 mx-auto">
        <button className='btn btn-info' onClick={makedeleteprocess}><i className='fas fa-trash'></i> Delete The news</button>  
        </div>
        </div>    
        </div>
      )
